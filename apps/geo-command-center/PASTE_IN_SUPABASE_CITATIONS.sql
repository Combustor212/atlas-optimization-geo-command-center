...
-- Citation & NAP Consistency: tables + RLS
-- ============================================
-- If you get "function update_updated_at_column() does not exist", run the block below first.
-- Otherwise skip to the tables section.

-- Optional: create trigger function if not already present
...
-- Drop existing policies if re-running (avoid duplicate policy errors)
DROP POLICY IF EXISTS "Agency members manage nap_profiles" ON nap_profiles;
DROP POLICY IF EXISTS "Clients read nap_profiles for their locations" ON nap_profiles;
DROP POLICY IF EXISTS "Agency members manage citations" ON citations;
DROP POLICY IF EXISTS "Clients read citations for their locations" ON citations;
DROP POLICY IF EXISTS "Agency members manage citation_audits" ON citation_audits;
DROP POLICY IF EXISTS "Clients read citation_audits for their locations" ON citation_audits;
...
-- citations: admin/staff CRUD; client read-only for their locations
...
-- citation_audits: admin/staff CRUD; client read-only for their locations
...
-- ============================================
-- TRIGGER (nap_profiles.updated_at)
-- ============================================
DROP TRIGGER IF EXISTS update_nap_profiles_updated_at ON nap_profiles;
