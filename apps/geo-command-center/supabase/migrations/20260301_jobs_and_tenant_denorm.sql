-- Jobs and Tenant Denormalization
-- TASK A: Add agency_id to locations + child tables, backfill, triggers, indexes
-- TASK B: Job queue (jobs, job_runs)
-- TASK C: claim_jobs() atomic claim function
-- TASK D: RLS for jobs/job_runs

-- ============================================
-- TASK A — Tenant Denormalization
-- ============================================

-- 1) Add agency_id columns (nullable for backfill)
...
-- 4) Triggers: auto-set agency_id on INSERT/UPDATE
-- Drop first so migration is idempotent (PostgreSQL has no CREATE OR REPLACE TRIGGER)
...
-- Note: rankings(location_id, keyword, recorded_at desc) already exists as idx_rankings_location_keyword
...
-- jobs: agency members SELECT only; INSERT if agency_id matches and is_agency_member; no UPDATE/DELETE
...
-- job_runs: agency members SELECT via job's agency; no INSERT/UPDATE/DELETE (workers use service role)
...
-- NOTES
-- ============================================
-- A) Tenant denorm: agency_id enables fast agency-scoped queries without joining through clients.
--    Triggers keep agency_id in sync on INSERT/UPDATE. Backfill is safe for existing data.
-- B) Jobs: Workers call claim_jobs(worker_id, n) with service role (bypasses RLS) to claim and process.
--    Use idempotency_key to avoid duplicate job creation.
-- C) claim_jobs: SECURITY INVOKER; workers use service role key which bypasses RLS for UPDATE.
-- D) RLS: Agency members can create and view jobs; only service-role workers can update status/locks.
