 ...
 * GEO benchmark - MGO backend compatible.
 * Uses Geo Command Center's AI visibility when OPENAI_API_KEY is set.
 */
...
export async function GET(req: NextRequest) {
  ...
  if (!placeId) {
    return NextResponse.json(
      { error: 'Missing or invalid placeId parameter', message: 'placeId query parameter is required', details: { received: placeId } },
      { status: 400 }
    )
  }

  const radiusMeters = radiusParam ? parseInt(radiusParam, 10) : 5000
  const forceRefresh = forceParam === '1' || forceParam === 'true'

  if (isNaN(radiusMeters) || radiusMeters < 1000 || radiusMeters > 50000) {
    return NextResponse.json(
      { error: 'Invalid radius parameter', message: 'radius must be between 1000 and 50000 meters', details: { received: radiusParam } },
      { status: 400 }
    )
  }
  ...
  try {
      ...
      return NextResponse.json(
        {
          ...
          message: 'GEO benchmark requires OPENAI_API_KEY. Configure it in your environment.',
    ...
    console.error('[GEO Benchmark] Error:', err)
    return NextResponse.json(
      {
