...
/**
 * Cron: enqueue PLACE_DETAILS_REFRESH jobs (one per agency per day).
 * Heavy work moved to services/worker/placeDetailsRefreshHandler.ts (worker will call it).
 */
export async function GET(req: Request) {
  ...
  if (agencyErr || !agencies?.length) {
    return NextResponse.json({
      queued: true,
      job_type: 'PLACE_DETAILS_REFRESH',
      enqueued_count: 0,
  ...
  for (const agency of agencies) {
    const idempotencyKey = `PLACE_DETAILS_REFRESH:${agency.id}:${dateBucket}`
