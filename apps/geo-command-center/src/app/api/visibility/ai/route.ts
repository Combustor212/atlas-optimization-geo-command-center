...
import { validateBody, aiVisibilitySchema } from '@/lib/validation'
...
export async function POST(request: Request) {
  try {
    ...
    // 5. Insert the AI visibility record
    ...
    if (error) {
      console.error('Error inserting AI visibility:', error)
      return errors.internalError('Failed to create AI visibility record')
    }
