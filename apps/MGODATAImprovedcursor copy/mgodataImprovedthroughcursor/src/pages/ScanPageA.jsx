/**
 * /scan-a — Version A (Control)
 * Clean 2-field scan form optimized for mobile TikTok traffic.
 * Fields: Business Name (Smart Search) + Email
 * TikTok variant: 'scan-a'
 */
...
import { Check, Loader2, ArrowRight, Building, Mail, MapPin, AlertTriangle } from 'lucide-react';
...
export default function ScanPageA() {
  ...
  const handleScan = async () => {
    ...
    if (!businessName || businessName.trim().length < 2) { toast.error('Please enter a business name'); return; }
  ...
  return (
    <div className="min-h-screen bg-white flex flex-col">
      ...
      <div className="text-center pt-4 pb-2">
        <span className="text-xs text-slate-400 font-medium tracking-wide">Step 1 of 2</span>
      </div>
        ...
        <div className="w-full max-w-md">
          <div className="text-center mb-8">
            ...
            <p className="text-slate-500 text-sm sm:text-base">
              Free 30-second scan · Google + AI visibility score
            </p>
          </div>
          ...
          <div className="bg-white rounded-2xl shadow-xl border border-slate-100 p-6">
            <form onSubmit={(e) => { e.preventDefault(); handleScan(); }} className="space-y-4">
              ...
              <div ref={dropdownRef} className="relative">
                ...
                {placeId && (
                  <p className="mt-1 text-xs text-green-600 font-medium flex items-center gap-1">
                    <Check className="w-3 h-3" /> Business found — details captured
                  </p>
                )}
                {!placeId && !fallbackMode && businessName.length >= 2 && !showDropdown && !isLoadingSuggestions && (
                  <p className="mt-1 text-xs text-slate-500 flex items-center gap-1">
                    <AlertTriangle className="w-3 h-3 text-amber-500" /> Select from the dropdown above
                    <button type="button" onClick={() => setFallbackMode(true)} className="ml-1 text-blue-600 underline">or enter manually</button>
                  </p>
                )}
              </div>
              ...
              <button
                ...
                className={cn(
                  "w-full h-14 rounded-xl font-bold text-base text-white flex items-center justify-center gap-2 transition-all mt-2",
                  isReady && !isScanning
